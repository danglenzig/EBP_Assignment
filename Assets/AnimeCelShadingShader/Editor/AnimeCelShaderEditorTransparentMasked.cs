...
using UnityEditor;
#endif
public class AnimeCelShaderEditorTransparentMasked : ShaderGUI
{
	...
	Vector2 maskTextureTiling;
	Vector2 maskTextureOffset;
	...
	public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
	{
		...
		else
		{
			...
			if (texturesFoldout)
			{
				...
				EditorGUILayout.LabelField("Opacity Mask");
				EditorGUILayout.HelpBox("Mask must be black and white", MessageType.Info);
				EditorGUILayout.BeginHorizontal();
				EditorGUILayout.BeginVertical();
				EditorGUILayout.Space(15f);
				maskTextureTiling = EditorGUILayout.Vector2Field("Tiling", targetMat.GetTextureScale("_OpacityMask"));
				maskTextureOffset = EditorGUILayout.Vector2Field("Offset", targetMat.GetTextureOffset("_OpacityMask"));
				EditorGUILayout.EndVertical();
				maskTexture = EditorGUILayout.ObjectField(targetMat.GetTexture("_OpacityMask"), typeof(Texture2D), false, texturesLayout) as Texture2D;
				EditorGUILayout.EndHorizontal();
			}
			GUILayout.EndVertical();
			GUILayout.EndHorizontal();
			...
			if (EditorGUI.EndChangeCheck() || (Event.current.type == EventType.ValidateCommand && Event.current.commandName == "UndoRedoPerformed"))
			{
				...
				targetMat.SetTexture("_OpacityMask", maskTexture);
				targetMat.SetTextureScale("_OpacityMask", maskTextureTiling);
				targetMat.SetTextureOffset("_OpacityMask", maskTextureOffset);
			}
		}
	}
	string GetAssetPath()
