...
using UnityEditor;
#endif
public class AnimeCelShaderEditorDissolve : ShaderGUI
{
	...
	Texture2D dissolveGuideTexture;
	Vector2 dissolveGuideTiling;
	Vector2 dissolveGuideOffset;
	...
	void GetCache()
	{
		if (cache == false)
		{
			...
			//TitleImage
			titleTex = AssetDatabase.LoadAssetAtPath(GetAssetPath() + "Interface/TitleDiffuseDissolve.png", typeof(Texture2D)) as Texture2D;
			//ColorsImage
	...
	public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
	{
		...
		else
		{
			...
			#endregion
			#region Dissolve
			GUIContent dissolveFoldoutTitle = new GUIContent("Dissolve Options", outlineTex);
			GUILayout.BeginHorizontal("box");
			GUILayout.BeginVertical();
			dissolveFoldout = EditorGUILayout.Foldout(dissolveFoldout, dissolveFoldoutTitle, true, foldoutTitleStyle);
			GUILayout.Space(10);

			if (dissolveFoldout)
			{
				bool DB = true;
				if (targetMat.GetFloat("_DissolveByUVs") == 0) { DB = false; } else { DB = true; }
				dissolveByUVs = EditorGUILayout.Toggle("Dissolve by UVs", DB);
				dissolveAmount = EditorGUILayout.Slider("Dissolve Amount",targetMat.GetFloat("_DissolveAmount"), 0, 1f);
			}
			GUILayout.EndVertical();
			GUILayout.EndHorizontal();
			#endregion
			...
			if (texturesFoldout)
			{
				...
				EditorGUILayout.LabelField("Dissolve Guide Texture");
				EditorGUILayout.BeginHorizontal();
				EditorGUILayout.BeginVertical();
				EditorGUILayout.Space(15f);
				dissolveGuideTiling = EditorGUILayout.Vector2Field("Tiling", targetMat.GetTextureScale("_DissolveGuide"));
				dissolveGuideOffset = EditorGUILayout.Vector2Field("Offset", targetMat.GetTextureOffset("_DissolveGuide"));
				EditorGUILayout.EndVertical();
				dissolveGuideTexture = EditorGUILayout.ObjectField(targetMat.GetTexture("_DissolveGuide"), typeof(Texture2D), false, texturesLayout) as Texture2D;
				EditorGUILayout.EndHorizontal();
			}
			GUILayout.EndVertical();
			GUILayout.EndHorizontal();
			...
			if (EditorGUI.EndChangeCheck() || (Event.current.type == EventType.ValidateCommand && Event.current.commandName == "UndoRedoPerformed"))
			{
				...
				//Dissolve
				targetMat.SetFloat("_DissolveAmount", dissolveAmount);
				if (dissolveByUVs) { targetMat.SetFloat("_DissolveByUVs", 1); } else { targetMat.SetFloat("_DissolveByUVs", 0); }
				...
				targetMat.SetTexture("_DissolveGuide", dissolveGuideTexture);
				targetMat.SetTextureScale("_DissolveGuide", dissolveGuideTiling);
				targetMat.SetTextureOffset("_DissolveGuide", dissolveGuideOffset);
			}
		}
	}
	string GetAssetPath()
